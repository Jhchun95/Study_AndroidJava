...
//      Toast.makeText(getApplicationContext(), "MainActivity onCreate() 실행", Toast.LENGTH_SHORT).show();
...
//  액티비티가 최초로 실행될 때 또는 다른 액티비티가 종료되서 현재 액티비티가 다시 화면에 나타날 때
    protected void onStart() {
        super.onStart();
//      Toast.makeText(getApplicationContext(), "MainActivity onStart() 실행", Toast.LENGTH_SHORT).show();
...
//  onPause() : 다른 액티비티가 실행되서 현재 액티비티를 가리기 시작하거나 현재 액티비티가 종료되기 시작하는 순간 실행된다.
    ...
    @Override
//  onStop() : 다른 액티비티가 실행되서 현재 액티비티를 완전히 가려서 더 이상 보이지 않거나 현재 액티비티가 종료되서 더 이상
//  화면에 보이지 않는 순간 실행된다.
    ...
    @Override
//  onDestroy : 프로그램이 종료될 때 딱 1번 실행된다.
