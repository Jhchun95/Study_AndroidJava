...
public class AppInjector {
    ...
    private static void handleActivity(Activity activity) {
        ...
        // Fragment Component 에서 AndroidSupportInjector을 사용하였기에 Fragment에서는 inject을 호출만으로 바인딩할 수 있다.
            ...
            ((FragmentActivity) activity).getSupportFragmentManager().registerFragmentLifecycleCallbacks(
                    ...
                    public void onFragmentCreated(FragmentManager fm, Fragment f, Bundle savedInstanceState) {
                                    if (f instanceof Injectable) {
                                        ...
                                        // The injector only contains the provision method for the field in the fragment.
...
// annotation : java의 소스코드를 설명하는 메타데이터
// ((FragmentActivity) activity).getSupportFragmentManager().registerFragmentLifecycleCallbacks :
//          Callback interface for listening to fragment state changes that happen within a given FragmentManager.

// inject : Dagger와 연관있고, 의존성 주입(DI, Dependency inject)을 요청한다.
// 안드로이드에서는 프레임워크 구조상 Application 단에서 @Component를 구성하기 때문에 각각 @Subcomponent인 Activitiy에서
// Component 호출 -> SubComponent 빌드 -> inject하는 코드들을 작성해야한다. 이것을 AndroidInjector.inject 한 줄로 간소화시킨다.
